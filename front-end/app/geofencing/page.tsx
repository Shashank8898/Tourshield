...
// Mock the geofencing hook (replace with actual import)
const useGeofencing = () => {
  ...
  const joinRoom = async (roomId) => {
    ...
    if (room) {
      ...
      setRoomMembers([
        ...
        { id: 2, name: 'You', isCreator: false, isOnline: true, location: userLocation }
      ]);
      setIsTracking(true);
    }
  ...
  const sendEmergencySOS = async (message) => {
    const alert = {
      id: Date.now(),
      type: 'emergency-sos',
  ...
  return {
    ...
    sendEmergencySOS,
    setIsTracking,
    setUserLocation // Add this to expose the setter
  };
};
...
const IntegratedGeoFencing = () => {
  ...
  useEffect(() => {
    alerts.forEach(alert => {
      if (alert.type === 'geo-fence-breach') {
        showNotificationPopup('Geo-fence Alert', alert.message, 'warning');
      } else if (alert.type === 'emergency-sos') {
        showNotificationPopup('Emergency Alert', alert.message, 'critical');
      }
    });
  }, [alerts]);
  ...
  // Location tracking with geo-fence checking
  useEffect(() => {
    let watchId = null;
    ...
    if (isTracking && navigator.geolocation) {
      watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          // For demo, use a fixed location near the geo-fence
          const demoLocation = currentRoom ? {
            lat: currentRoom.location.lat + (Math.random() - 0.5) * 0.01,
            lng: currentRoom.location.lng + (Math.random() - 0.5) * 0.01,
            accuracy: 10,
            timestamp: Date.now()
          } : location;
          ...
          // Check geo-fence breach (mock)
          if (currentRoom && Math.random() > 0.95) { // 5% chance for demo
            const distance = Math.floor(Math.random() * 500 + currentRoom.radius);
            const alert = {
              id: Date.now(),
              type: 'geo-fence-breach',
              message: `You've moved outside the safe zone! Distance: ${distance}m`,
              timestamp: new Date().toLocaleTimeString(),
              severity: 'high'
            };
  ...
  const showNotificationPopup = (title, message, type) => {
  ...
  const handleEmergencySOS = async () => {
    try {
      await sendEmergencySOS(sosMessage);
          ...
          <div className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-3xl p-8">
            ...
            <div className="space-y-6">
              ...
              <button
                ...
                className="w-full py-4 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-2xl font-bold text-lg hover:shadow-2xl hover:shadow-cyan-500/25 transform hover:scale-105 transition-all duration-500 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Create Safety Room
              </button>
            </div>
